...
function SkillsSection({ supabase }) {
  ...
  const loadAll = async () => {
    ...
    (progressRes.data || []).forEach(p => {
      prog[`${p.skill_exercise_id}_${p.person}`] = p.current_stage_id;
    });
    setProgress(prog);
    setLoading(false);
  };
  ...
  const loadLogs = async (exerciseId) => {
    const { data } = await supabase
      .from('skill_logs')
      .select('*, skill_stages(name, target_value, target_unit)')
      .eq('skill_exercise_id', exerciseId)
  ...
  const getExStages = (exerciseId) =>
    stages.filter(s => s.skill_exercise_id === exerciseId).sort((a, b) => a.stage_number - b.stage_number);

  const getCurrentStage = (exerciseId, person) => {
    const stageId = progress[`${exerciseId}_${person}`];
    if (!stageId) return getExStages(exerciseId)[0];
    return stages.find(s => s.id === stageId);
  };

  const getNextStage = (exerciseId, currentStageNumber) =>
    getExStages(exerciseId).find(s => s.stage_number > currentStageNumber) || null;
  ...
  const isFinalStage = (exerciseId, stageNumber) => {
    const exStages = getExStages(exerciseId);
    return exStages[exStages.length - 1]?.stage_number === stageNumber;
  };
  ...
  const openExercise = async (ex) => {
    setSelectedEx(ex);
    await loadLogs(ex.id);
    setView('detail');
    setOpenInstructions(null);
    setLogForm({ person: 'You', value: '', sets: 3, notes: '' });
  };
  ...
  const saveLog = async () => {
    ...
    const { error } = await supabase.from('skill_logs').insert({
      skill_exercise_id: selectedEx.id,
      stage_id: stage?.id,
      person: logForm.person,
      achieved_value: parseFloat(logForm.value),
      sets_completed: parseInt(logForm.sets) || 1,
      notes: logForm.notes || null,
      logged_date: localDate(),
    });
    if (error) { showToast('Error saving', 'error'); console.error(error); }
    else {
      await loadLogs(selectedEx.id);
      setLogForm(f => ({ ...f, value: '', notes: '' }));
      showToast('Logged!');
    }
    setSaving(false);
  };
  ...
  const advanceStage = async (person) => {
    ...
    const { error } = await supabase.from('skill_progress').upsert(
      { skill_exercise_id: selectedEx.id, person, current_stage_id: next.id },
      { onConflict: 'skill_exercise_id,person' }
    );
    if (!error) {
      setProgress(p => ({ ...p, [`${selectedEx.id}_${person}`]: next.id }));
      showToast(`${person} → ${next.name}`);
    }
  };
  ...
  const sc = {
    ...
    stageName:    { fontSize: 13, fontWeight: 700, marginBottom: 3 },
    stageTarget:  { color: '#555', fontSize: 12 },
  ...
  if (view === 'list') {
    return (
      <div style={sc.page}>
        ...
        {!loading && exercises.length === 0 && <div style={sc.empty}>No skills found. Run skills_full_library.sql in Supabase first.</div>}
        {exercises.map(ex => {
            ...
            <div key={person} style={{ marginBottom: 16 }}>
              ...
              <div style={sc.stageBlock}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ ...sc.stageName, color: nameColor }}>Stage {stage.stage_number} — {stage.name}</div>
                    <div style={sc.stageTarget}>Target: {stage.target_value} {stage.target_unit} × {stage.target_sets} sets</div>
                  </div>
...
function CaliWorkoutsSection({ supabase }) {
  ...
  const getExercises = (templateId) =>
    exercises.filter(e => e.template_id === templateId).sort((a, b) => a.order_num - b.order_num);

  const lastDoneDate = (templateId, p) => {
    const entry = logs.find(l => l.template_id === templateId && l.person === p);
    return entry ? entry.logged_date : null;
  };

  const logWorkout = async () => {
    if (!selected) return;
    setLogging(true);
    const { error } = await supabase.from('cali_workout_logs').insert({
  ...
  const focusColor = (focus) => ({
    push: { bg: '#1a0d0d', border: '#4a1a1a', text: '#f07070' },
    pull: { bg: '#0d1a0d', border: '#1a4a1a', text: '#70c070' },
    core: { bg: '#0d0d1a', border: '#1a1a4a', text: '#7070f0' },
  })[focus] || { bg: '#1a1a1a', border: '#2a2a2a', text: '#aaa' };
    ...
    return (
      <div style={wc.page}>
        ...
        {exs.map((ex, i) => {
          ...
          return (
            <div key={ex.id} style={{ ...wc.exCard, background: isWarmup || isLast ? '#111' : '#161616', borderColor: '#242424' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
...
export default function App(){
  ...
  const[selDay,setSelDay]=useState(null);const[week,setWeek]=useState(()=>cache.get("week")||12);const[loading,setLoading]=useState(true);
  ...
  async function load(){setLoading(true);try{
    ...
    const{data:tg}=await supabase.from("macro_targets").select("*").eq("is_active",true).limit(1);if(tg?.[0]){const t={protein:tg[0].protein_g_target,carbs:tg[0].carbs_g_target,fat:tg[0].fat_g_target,calories:tg[0].calories_target,goalName:tg[0].goal_name,bw:tg[0].bodyweight_lb};setMt(t);cache.set("mt",t);}
...
function Fuel({foods,setFoods,mt,setMt,meas=[],online,onPC}){
  ...
  function recalc(){
    ...
    const bf=parseFloat(calcBF)||20;const leanMass=w*(1-bf/100);
    const act=ACTIVITY.find(a=>a.name===calcAct)||ACTIVITY[2];const goal=GOALS.find(g=>g.name===calcG)||GOALS[1];
    const tdee=useEmpirical?parseFloat(empiricalMaint)||3100:calcTDEE(w,h,age,act.mult);
    const cal=Math.round(tdee+goal.delta);const protein=Math.round(leanMass*(parseFloat(calcP)||1.18));
    const fat=Math.max(70,Math.round(leanMass*(parseFloat(calcF)||0.37)));const carbs=Math.max(0,Math.round((cal-protein*4-fat*9)/4));
    setMt({protein,carbs,fat,calories:cal,goalName:calcG,bw:w});cache.set("mt",{protein,carbs,fat,calories:cal,goalName:calcG,bw:w});
  ...
  return(
    <div style={{padding:"24px 16px"}}>
      ...
      {showCalc&&(()=>{const prev=previewMacros();return(<div style={{...card,marginBottom:14}}>
        ...
        {useEmpirical?(<div style={{marginBottom:12}}><div style={{...hlbl,marginBottom:4}}>Calories currently maintaining on</div><input type="number" value={empiricalMaint} onChange={e=>setEmpiricalMaint(e.target.value)} style={{...inp,fontSize:16}}/><div style={{fontSize:10,color:C.mt,marginTop:5}}>Based on your actual weight trend — more accurate than any formula</div></div>):(<div style={{marginBottom:12}}><div style={{...hlbl,marginBottom:6}}>Activity level</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{ACTIVITY.map(a=><button key={a.name} onClick={()=>setCalcAct(a.name)} style={{padding:"5px 10px",borderRadius:8,border:`1px solid ${calcAct===a.name?C.ac:C.bd}`,background:calcAct===a.name?`${C.ac}15`:"transparent",color:calcAct===a.name?C.ac:C.mt,fontSize:11,fontWeight:calcAct===a.name?600:400,cursor:"pointer"}}>{a.name}<span style={{fontSize:9,color:C.mt,display:"block"}}>{a.label}</span></button>)}</div></div>)}
      ...
      {showAI&&(<div style={{...card,marginBottom:12}}>
        ...
        {aiResult&&(<div style={{background:C.sf2,borderRadius:10,border:`1px solid ${C.gn}33`,padding:12}}>
          ...
          <div style={{display:"flex",gap:6}}><button onClick={()=>logAIResult(false)} style={{...btnS,flex:1}}>Log only</button><button onClick={()=>logAIResult(true)} style={{...btnP,flex:1,fontSize:12}}>Log + save to DB</button></div>
        </div>)}
      </div>)}
      <div style={{display:"flex",gap:8,marginBottom:12}}>
...
function Body({meas,onAdd,online,onPC}){
  ...
  return(
    <div style={{padding:"24px 16px"}}>
      ...
      {lat&&(<div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
          <div style={{background:C.sf,borderRadius:12,border:`1px solid ${C.bd}`,padding:"16px 14px"}}><div style={{...hlbl,marginBottom:6}}>Bodyweight</div><div style={{fontSize:28,fontWeight:800,fontFamily:mono,lineHeight:1}}>{lat.bodyweight_lb??<span style={{color:C.mt}}>—</span>}</div><div style={{fontSize:11,color:C.mt,marginTop:4}}>lb{wtDelta&&<span style={{color:parseFloat(wtDelta)>0?C.am:C.gn,marginLeft:5}}>{wtDelta}</span>}</div></div>
          <div style={{background:C.sf,borderRadius:12,border:`1px solid ${C.ac}22`,padding:"16px 14px"}}><div style={{...hlbl,marginBottom:6}}>Body fat</div><div style={{fontSize:28,fontWeight:800,fontFamily:mono,color:C.ac,lineHeight:1}}>{latBF??<span style={{color:C.mt,fontSize:20}}>—</span>}</div><div style={{fontSize:11,color:C.mt,marginTop:4}}>%{bfDelta&&<span style={{color:parseFloat(bfDelta)>0?C.rd:C.gn,marginLeft:5}}>{bfDelta}</span>}</div></div>
        </div>
        ...
        <div style={{background:C.sf,borderRadius:12,border:`1px solid ${C.bd}`,padding:14,marginBottom:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
            {[{l:"Chest",v:lat.chest_in,p:prev?.chest_in},{l:"Waist",v:lat.waist_in,p:prev?.waist_in},{l:"Hips",v:lat.hips_in,p:prev?.hips_in},{l:"R Arm",v:lat.r_arm_in,p:prev?.r_arm_in},{l:"L Arm",v:lat.l_arm_in,p:prev?.l_arm_in},{l:"Shoulders",v:lat.shoulder_circ_in,p:prev?.shoulder_circ_in},{l:"R Forearm",v:lat.r_forearm_in,p:prev?.r_forearm_in},{l:"L Forearm",v:lat.l_forearm_in,p:prev?.l_forearm_in},{l:"Thigh",v:lat.thigh_in,p:prev?.thigh_in},{l:"Calf",v:lat.calf_in,p:prev?.calf_in},{l:"Neck",v:lat.neck_in,p:prev?.neck_in}].map(s=>{const d=delta(s.v,s.p);return(<div key={s.l}><div style={{fontSize:8,color:C.mt,textTransform:"uppercase",letterSpacing:"0.06em"}}>{s.l}</div><div style={{fontSize:16,fontWeight:700,fontFamily:mono,marginTop:2}}>{s.v??<span style={{color:C.mt,fontSize:12}}>—</span>}</div>{s.v&&<div style={{fontSize:9,color:C.mt}}>"{ }{ }{d&&<span style={{color:parseFloat(d)>0?C.gn:C.rd,marginLeft:2}}>{d}</span>}</div>}</div>);})}
          </div>
        </div>
      </div>)}
      ...
      {[...meas].reverse().map(m=>{const bf=m.body_fat_pct||(m.waist_in&&m.neck_in&&m.height_in?navyBF(m.waist_in,m.neck_in,m.height_in):null);return(<div key={m.id} style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:"10px 14px",marginBottom:5,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:10,color:C.mt}}>{m.measure_date}</div><div style={{fontSize:16,fontWeight:700,fontFamily:mono}}>{m.bodyweight_lb} lb</div></div><div style={{fontSize:10,color:C.mt,fontFamily:mono,textAlign:"right"}}>{bf&&<div style={{color:C.ac,marginBottom:2}}>BF {bf}%</div>}<div>{m.chest_in?`Ch ${m.chest_in}″`:""}{m.r_arm_in?` · A ${m.r_arm_in}″`:""}</div></div></div>);})}
    </div>
  );
}
  ...
  return(
    <div style={{padding:"24px 16px"}}>
      ...
      {prs.length>0&&(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
        {topPR&&<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.gn}22`,padding:"12px 14px"}}><div style={{...hlbl,color:C.gn,marginBottom:5}}>Top E1RM</div><div style={{fontSize:18,fontWeight:700,fontFamily:mono,color:C.gn}}>{Math.round(topPR.est1rm)}<span style={{fontSize:10,color:C.mt,fontWeight:400}}> lb</span></div><div style={{fontSize:10,color:C.mt,marginTop:2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{topPR.exercise}</div></div>}
        {topRaw&&<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.ac}22`,padding:"12px 14px"}}><div style={{...hlbl,color:C.ac,marginBottom:5}}>Heaviest set</div><div style={{fontSize:18,fontWeight:700,fontFamily:mono,color:C.ac}}>{topRaw.weight}<span style={{fontSize:10,color:C.mt,fontWeight:400}}> lb</span></div><div style={{fontSize:10,color:C.mt,marginTop:2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{topRaw.exercise}</div></div>}
      </div>)}
