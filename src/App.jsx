...
function Session({day,onBack,week,restDur,weekType,isDeload,online,onPC,activeProgram}){
  const[expEx,setExpEx]=useState(0);const[showCues,setShowCues]=useState(null);const[sd,setSd]=useState({});const[sid,setSid]=useState(null);const[saved,setSaved]=useState(null);const[showTimer,setShowTimer]=useState(false);const[timerKey,setTimerKey]=useState(0);const[lw,setLw]=useState({});const[history,setHistory]=useState(null);const[notes,setNotes]=useState("");const[notesSaved,setNotesSaved]=useState(false);
  ...
  return(
    <div style={{padding:"24px 16px"}}>
      ...
      {showCalc&&(()=>{const prev=previewMacros();return(<div style={{...card,marginBottom:14}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:12}}><div><div style={{...hlbl,marginBottom:4}}>Weight (lb)</div><input type="number" value={calcW} onChange={e=>setCalcW(e.target.value)} style={{...inp,fontSize:13}}/></div><div><div style={{...hlbl,marginBottom:4}}>Height (in)</div><input type="number" value={calcH} onChange={e=>setCalcH(e.target.value)} style={{...inp,fontSize:13}}/></div><div><div style={{...hlbl,marginBottom:4}}>Age</div><input type="number" value={calcAge} onChange={e=>setCalcAge(e.target.value)} style={{...inp,fontSize:13}}/></div><div><div style={{...hlbl,marginBottom:4}}>Body Fat %</div><input type="number" value={calcBF} onChange={e=>setCalcBF(e.target.value)} style={{...inp,fontSize:13}}/></div></div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}><div><div style={{...hlbl,marginBottom:4}}>Protein / lean lb</div><input type="number" value={calcP} onChange={e=>setCalcP(e.target.value)} style={{...inp,fontSize:13}}/></div><div><div style={{...hlbl,marginBottom:4}}>Fat / lean lb</div><input type="number" value={calcF} onChange={e=>setCalcF(e.target.value)} style={{...inp,fontSize:13}}/></div></div>
