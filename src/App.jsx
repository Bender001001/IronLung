...
function getPending(){return cache.get("pending")||[];}function addPending(op){const q=getPending();q.push({...op,ts:Date.now()});cache.set("pending",q);}
...
function Timer({duration,onDismiss}){
  ...
  useEffect(()=>{
    function tick(){
      ...
      setRem(left);
      if(left<=0&&!done){setDone(true);if(navigator.vibrate)navigator.vibrate([200,100,200,100,200]);}
    }
    tick(); // immediate check (catches background return)
    ref.current=setInterval(tick,500); // check every 500ms for accuracy
  ...
  return(<div style={{background:done?C.gn+"12":C.sf2,border:`1px solid ${done?C.gn+"33":C.bd}`,borderRadius:10,padding:"10px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:12}}>
    <div style={{flex:1}}>
      ...
      <div style={{width:"100%",height:2,background:C.bd,borderRadius:1,marginTop:6,overflow:"hidden"}}>
        <div style={{width:`${Math.min(pct,100)}%`,height:"100%",background:done?C.gn:C.ac,borderRadius:1,transition:"width 0.5s linear"}}/>
      </div>
    </div>
    <div style={{display:"flex",gap:4}}>
      {!done&&<button onClick={()=>addTime(30)} style={{padding:"5px 8px",background:C.sf,border:`1px solid ${C.bd}`,borderRadius:6,color:C.mt,fontSize:11,cursor:"pointer"}}>+30s</button>}
...
export default function App(){
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",color:C.tx,fontFamily:sans,maxWidth:480,margin:"0 auto",paddingBottom:80}}>
      {(!online||pc>0)&&<div style={{background:!online?C.am+"15":C.gn+"12",borderBottom:`1px solid ${!online?C.am+"33":C.gn+"33"}`,padding:"6px 16px"}}><span style={{fontSize:11,color:!online?C.am:C.gn}}>{!online?"Offline mode":pc>0?`Syncing ${pc}...`:"Synced"}</span></div>}
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:C.sf,borderTop:`1px solid ${C.bd}`,display:"flex",zIndex:100,padding:"6px 0 env(safe-area-inset-bottom, 4px)"}}>
        {tabs.map(t=>{const active=tab===t.id;const color=active?C.ac:C.mt;return(
          <button key={t.id} onClick={()=>{setTab(t.id);if(t.id!=="train")setSelDay(null);}}
            style={{flex:1,padding:"8px 0",background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,minHeight:48,justifyContent:"center",position:"relative"}}>
            {active&&<div style={{position:"absolute",top:0,left:"25%",right:"25%",height:2,background:C.ac,borderRadius:1}}/>}
            <t.Icon c={color}/><span style={{fontSize:10,fontWeight:active?700:500,color,letterSpacing:"0.04em",fontFamily:sans}}>{t.label}</span>
          </button>);})}
      </div>
    </div>);
}
...
function DaySelect({days,onSelect,week,setWeek,restDur,setRestDur,weekType,setWeekType,online}){
  ...
  return(<div style={{padding:"24px 16px"}}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><span style={{fontSize:12,color:isDL?C.am:C.mt}}>{weekType}</span>{wc!==null&&<span style={{fontSize:11,fontFamily:mono,color:wc===100?C.gn:wc>0?C.am:C.mt}}>{wc}%</span>}{summary&&<button onClick={()=>setShowSum(!showSum)} style={{marginLeft:"auto",padding:"4px 10px",background:C.sf,border:`1px solid ${C.bd}`,borderRadius:6,color:showSum?C.ac:C.mt,fontSize:10,cursor:"pointer"}}>Summary</button>}</div>
    {showSum&&summary&&(<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:14,marginBottom:14}}>
      <div style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase",marginBottom:10}}>Week {week} Recap</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:12}}>{[{l:"Sessions",v:summary.sessionsLogged,c:C.ac},{l:"Sets Done",v:summary.completedSets,c:C.bl},{l:"Total",v:summary.totalSets,c:C.mt},{l:"PRs",v:summary.prCount,c:summary.prCount>0?C.gn:C.mt}].map(s=><div key={s.l} style={{textAlign:"center"}}><div style={{fontSize:18,fontWeight:700,fontFamily:mono,color:s.c}}>{s.v}</div><div style={{fontSize:8,color:C.mt,textTransform:"uppercase",marginTop:2}}>{s.l}</div></div>)}</div>
      {Object.keys(summary.muscles).length>0&&<div><div style={{fontSize:9,color:C.mt,textTransform:"uppercase",marginBottom:6}}>Volume by Muscle</div><div style={{display:"flex",flexWrap:"wrap",gap:4}}>{Object.entries(summary.muscles).sort((a,b)=>b[1]-a[1]).map(([m,sets])=>{const tgt=VOL_TARGETS[m];const inR=tgt&&sets>=tgt.min&&sets<=tgt.max;return <span key={m} style={{padding:"3px 8px",borderRadius:4,background:C.sf2,border:`1px solid ${inR?C.gn+"33":C.bd}`,fontSize:10,fontFamily:mono}}><span style={{color:C.tx}}>{m}</span> <span style={{color:inR?C.gn:C.mt}}>{sets}</span></span>;})}</div></div>}
    </div>)}
    {showCfg&&(<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:14,marginBottom:14}}>
      <div style={{marginBottom:12}}><div style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase",marginBottom:8}}>Week Phase</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{WEEK_TYPES.map(t=>(<button key={t} onClick={()=>setWeekType(t)} style={{padding:"6px 12px",borderRadius:6,border:`1px solid ${weekType===t?t==="Deload"?C.am:C.ac:C.bd}`,background:weekType===t?(t==="Deload"?C.am:C.ac)+"15":"transparent",color:weekType===t?(t==="Deload"?C.am:C.ac):C.mt,fontSize:12,cursor:"pointer"}}>{t}</button>))}</div>{isDL&&<div style={{fontSize:11,color:C.am,marginTop:8}}>2 sets at ~60%</div>}</div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      {ROTATION.map((dn,i)=>{if(dn==="Rest")return<div key={`r${i}`} style={{padding:"12px 16px",background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,opacity:0.4}}><span style={{fontSize:12,color:C.mt}}>Day {i+1} — Rest</span></div>;
        ...
        return(<button key={dn} onClick={()=>onSelect(day)} style={{padding:"14px 16px",background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:14}}><div style={{fontFamily:mono,fontSize:13,fontWeight:600,color:C.mt,width:20}}>{i+1}</div><div style={{flex:1}}><div style={{fontSize:14,fontWeight:600}}>{day.name}</div><div style={{fontSize:11,color:C.mt,marginTop:2}}>{day.focus}</div></div><span style={{color:C.mt+"88",fontSize:14}}>›</span></button>);
      })}
    </div>
  </div>);
}
...
function Session({day,onBack,week,restDur,weekType,isDeload,online,onPC}){
  ...
  async function sv(eid,sn){if(!sid)return;const k=`${eid}-${sn}`,d=sd[k];if(!d||(!d.weight&&!d.reps))return;const ck=`session_${day.id}_${week}`;const cached=cache.get(ck)||{sid,sets:{}};cached.sets[k]={weight:d.weight,reps:d.reps,dbId:d.dbId};cache.set(ck,cached);
    ...
    // NO auto-timer — user starts it manually
  }
  ...
  return(<div style={{padding:"20px 16px"}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      {day.exercises.map((ex,xi)=>{const es=eff(ex),isE=expEx===xi,dn=done(ex.id,es),all=dn===es,pg=lw[ex.id];
        // Calculate what weight to use today
        const todayWeight = pg ? (pg.deload ? pg.sw : pg.up ? pg.sw : pg.w) : null;
        const todayLabel = pg ? (pg.deload ? `${pg.sw}lb (60% of ${pg.w})` : pg.up ? `${pg.sw}lb (${pg.w} + ${ex.increment})` : `${pg.w}lb (same)`) : null;

        return(<div key={ex.id} style={{background:C.sf,borderRadius:10,border:`1px solid ${all?C.gn+"28":C.bd}`,overflow:"hidden"}}>
          <button onClick={()=>{setExpEx(isE?-1:xi);setHistory(null);setShowTimer(false);}} style={{width:"100%",padding:"12px 14px",background:"none",border:"none",color:C.tx,cursor:"pointer",display:"flex",alignItems:"center",gap:10,textAlign:"left"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:11,color:C.mt,marginTop:1}}>
                {es}×{ex.repMin}–{ex.repMax}
                {todayWeight && <span style={{color:pg.up?C.gn:pg.deload?C.am:C.mt}}> · {todayWeight}lb</span>}
                {dn>0&&<span style={{color:all?C.gn:C.am}}> · {dn}/{es}</span>}
              </div>
            </div>
            {pg?.up&&!all&&!isDeload&&<span style={{fontSize:9,fontWeight:600,color:C.gn,background:C.gn+"12",padding:"2px 6px",borderRadius:4}}>↑</span>}
          ...
          {isE&&(<div style={{padding:"0 14px 14px"}}>
            <div style={{display:"flex",gap:6,marginBottom:8}}>
              <button onClick={()=>setShowCues(showCues===xi?null:xi)} style={{flex:1,padding:"7px 10px",background:C.sf2,border:`1px solid ${C.bd}`,borderRadius:6,color:C.mt,fontSize:11,cursor:"pointer",textAlign:"left"}}>{showCues===xi?ex.cues:"View cues"}</button>
              {ex.video&&<a href={ex.video} target="_blank" rel="noopener noreferrer" style={{padding:"7px 10px",background:C.sf2,border:`1px solid ${C.bd}`,borderRadius:6,color:C.ac,fontSize:11,textDecoration:"none"}}>Watch</a>}
              <button onClick={()=>history?.exerciseId===ex.id?setHistory(null):loadHistory(ex.id)} style={{padding:"7px 10px",background:C.sf2,border:`1px solid ${history?.exerciseId===ex.id?C.ac:C.bd}`,borderRadius:6,color:history?.exerciseId===ex.id?C.ac:C.mt,fontSize:11,cursor:"pointer"}}>History</button>
            </div>

            {history?.exerciseId===ex.id&&history.weeks.length>0&&(
              <div style={{background:C.sf2,borderRadius:8,padding:10,marginBottom:8}}>
                <div style={{fontSize:9,color:C.mt,textTransform:"uppercase",marginBottom:8}}>Weight progression</div>
                <div style={{display:"flex",alignItems:"flex-end",gap:3,height:50}}>
                  {history.weeks.slice(-12).map(w=>{const mn=Math.min(...history.weeks.map(x=>x.weight)),mx=Math.max(...history.weeks.map(x=>x.weight)),rn=mx-mn||1,h=((w.weight-mn)/rn)*40+8;
                    return(<div key={w.week} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}><span style={{fontSize:7,fontFamily:mono,color:C.tx}}>{w.weight}</span><div style={{width:"100%",height:h,background:w.week===week?C.ac:C.ac+"44",borderRadius:2,maxWidth:24}}/><span style={{fontSize:6,color:C.mt}}>W{w.week}</span></div>);})}
                </div>
              </div>
            )}
            ...
            {pg&&(
              <div style={{padding:"8px 12px",marginBottom:8,background:pg.deload?C.am+"08":pg.up?C.gn+"08":C.sf2,border:`1px solid ${pg.deload?C.am+"22":pg.up?C.gn+"22":C.bd}`,borderRadius:8}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:10,color:C.mt}}>Today's weight</div>
                    <div style={{fontSize:18,fontWeight:700,fontFamily:mono,color:pg.deload?C.am:pg.up?C.gn:C.tx}}>{todayWeight}lb</div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:10,color:C.mt}}>{pg.deload?"Deload 60%":pg.up?`+${ex.increment}lb progression`:"Hold weight"}</div>
                    <div style={{fontSize:11,color:C.mt,fontFamily:mono}}>Last: {pg.w}lb · {pg.r.toFixed(1)}avg</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>
              <button onClick={()=>fill(ex.id,es,todayWeight||0)} style={{padding:"5px 10px",background:pg?.up?C.gn+"10":pg?.deload?C.am+"10":C.sf2,border:`1px solid ${pg?.up?C.gn+"33":pg?.deload?C.am+"33":C.bd}`,borderRadius:6,color:pg?.up?C.gn:pg?.deload?C.am:C.mt,fontSize:11,fontWeight:600,cursor:"pointer"}}>Fill {todayWeight||0}lb</button>
              {pg?.up&&<button onClick={()=>fill(ex.id,es,pg.w)} style={{padding:"5px 10px",background:C.sf2,border:`1px solid ${C.bd}`,borderRadius:6,color:C.mt,fontSize:11,cursor:"pointer"}}>Keep {pg.w}lb</button>}
            </div>

            {/* Manual timer */}
            {showTimer?<Timer key={timerKey} duration={restDur} onDismiss={()=>setShowTimer(false)}/>
            :<button onClick={startTimer} style={{width:"100%",padding:"8px",marginBottom:8,background:C.sf2,border:`1px solid ${C.bd}`,borderRadius:8,color:C.mt,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.mt} strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
            ...
            {xi<day.exercises.length-1&&<button onClick={()=>{setExpEx(xi+1);setShowTimer(false);setHistory(null);}} style={{width:"100%",padding:"9px",marginTop:4,background:C.ac+"10",border:`1px solid ${C.ac}25`,borderRadius:8,color:C.ac,fontSize:12,fontWeight:600,cursor:"pointer"}}>Next exercise</button>}
          </div>)}
        </div>);
      })}
    </div>
...
function Fuel({foods,setFoods,mt,setMt,online,onPC}){const[log,setLog]=useState([]);const[search,setSearch]=useState("");const[showS,setShowS]=useState(false);const[cat,setCat]=useState("All");const[showCalc,setShowCalc]=useState(false);const[showAdd,setShowAdd]=useState(false);const[showRecent,setShowRecent]=useState(false);const[calcW,setCalcW]=useState("205");const[calcG,setCalcG]=useState("Lean Bulk");const[calcP,setCalcP]=useState("0.85");const[calcF,setCalcF]=useState("0.35");const[nf,setNf]=useState({name:"",portion_size:"",portion_unit:"",protein_g:"",carbs_g:"",fat_g:"",calories:"",category:"Protein"});const[recentFoods,setRecentFoods]=useState([]);const[td]=useState(new Date().toISOString().split("T")[0]);
  ...
  return(<div style={{padding:"24px 16px"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}><div><div style={{fontSize:20,fontWeight:700}}>Fuel</div><div style={{fontSize:11,color:C.mt}}>{mt.calories} cal target</div></div><button onClick={()=>setShowCalc(!showCalc)} style={{padding:"6px 12px",background:C.sf,border:`1px solid ${C.bd}`,borderRadius:6,color:showCalc?C.ac:C.mt,fontSize:11,cursor:"pointer"}}>Calculator</button></div>
    {showCalc&&(<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:14,marginBottom:14}}><div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>{GOALS.map(g=>(<button key={g.name} onClick={()=>setCalcG(g.name)} style={{padding:"5px 10px",borderRadius:6,border:`1px solid ${calcG===g.name?C.ac:C.bd}`,background:calcG===g.name?C.ac+"15":"transparent",color:calcG===g.name?C.ac:C.mt,fontSize:11,cursor:"pointer"}}>{g.name}</button>))}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}><div><div style={{fontSize:9,color:C.mt,textTransform:"uppercase",marginBottom:4}}>Weight</div><input type="number" value={calcW} onChange={e=>setCalcW(e.target.value)} style={inp}/></div><div><div style={{fontSize:9,color:C.mt,textTransform:"uppercase",marginBottom:4}}>Pro/lb</div><input type="number" value={calcP} onChange={e=>setCalcP(e.target.value)} style={inp}/></div><div><div style={{fontSize:9,color:C.mt,textTransform:"uppercase",marginBottom:4}}>Fat/lb</div><input type="number" value={calcF} onChange={e=>setCalcF(e.target.value)} style={inp}/></div></div><button onClick={recalc} style={{width:"100%",padding:"10px",background:C.ac,border:"none",borderRadius:8,color:C.bg,fontSize:13,fontWeight:700,cursor:"pointer"}}>Update Targets</button></div>)}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:14}}>{[{l:"Pro",v:Math.round(tot.protein),t:mt.protein,u:"g",c:C.gn},{l:"Carb",v:Math.round(tot.carbs),t:mt.carbs,u:"g",c:C.bl},{l:"Fat",v:Math.round(tot.fat),t:mt.fat,u:"g",c:C.am},{l:"Cal",v:Math.round(tot.calories),t:mt.calories,u:"",c:C.ac}].map(m=>{const p=Math.round((m.v/m.t)*100);return(<div key={m.l} style={{background:C.sf,borderRadius:10,padding:"10px 6px",textAlign:"center",border:`1px solid ${C.bd}`}}><div style={{fontSize:16,fontWeight:700,fontFamily:mono,color:p>100?C.rd:m.c}}>{m.v}</div><div style={{fontSize:9,color:C.mt}}>/{m.t}{m.u}</div><div style={{width:"100%",height:2,background:C.bd,borderRadius:1,marginTop:5,overflow:"hidden"}}><div style={{width:`${Math.min(p,100)}%`,height:"100%",background:m.c,borderRadius:1}}/></div><div style={{fontSize:8,color:C.mt,marginTop:3,textTransform:"uppercase"}}>{m.l}</div></div>);})}</div>
    {recentFoods.length>0&&!showS&&!showAdd&&(<div style={{marginBottom:12}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase"}}>Recent</span><button onClick={()=>setShowRecent(!showRecent)} style={{background:"none",border:"none",color:C.ac,fontSize:10,cursor:"pointer"}}>{showRecent?"Less":"More"}</button></div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{(showRecent?recentFoods:recentFoods.slice(0,6)).map(f=>(<button key={f.id} onClick={()=>add(f,f.lastPortions||1)} style={{padding:"6px 10px",background:C.sf,border:`1px solid ${C.bd}`,borderRadius:8,color:C.tx,fontSize:11,cursor:"pointer",display:"flex",gap:6,alignItems:"center"}}><span>{f.name.length>18?f.name.slice(0,18)+"…":f.name}</span><span style={{fontFamily:mono,fontSize:10,color:C.gn}}>{f.protein_g}p</span></button>))}</div></div>)}
    <div style={{display:"flex",gap:8,marginBottom:12}}><button onClick={()=>{setShowS(!showS);setShowAdd(false);}} style={{flex:1,padding:"10px",background:showS?C.sf2:C.ac+"10",border:`1px solid ${showS?C.bd:C.ac}25`,borderRadius:8,color:showS?C.mt:C.ac,fontSize:13,fontWeight:600,cursor:"pointer"}}>{showS?"Close":"Search foods"}</button><button onClick={()=>{setShowAdd(!showAdd);setShowS(false);}} style={{padding:"10px 14px",background:showAdd?C.sf2:C.sf,border:`1px solid ${showAdd?C.ac:C.bd}`,borderRadius:8,color:showAdd?C.ac:C.mt,fontSize:12,cursor:"pointer"}}>+ New</button></div>
    {showAdd&&(<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:14,marginBottom:12}}><div style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase",marginBottom:10}}>Custom Food</div><input type="text" value={nf.name} onChange={e=>setNf(p=>({...p,name:e.target.value}))} placeholder="Food name" style={{...inpL,marginBottom:8}}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}><input type="number" inputMode="decimal" value={nf.portion_size} onChange={e=>setNf(p=>({...p,portion_size:e.target.value}))} placeholder="Portion" style={inp}/><input type="text" value={nf.portion_unit} onChange={e=>setNf(p=>({...p,portion_unit:e.target.value}))} placeholder="Unit" style={inpL}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,marginBottom:8}}>{[{k:"protein_g",l:"Pro"},{k:"carbs_g",l:"Carb"},{k:"fat_g",l:"Fat"},{k:"calories",l:"Cal"}].map(f=>(<div key={f.k}><div style={{fontSize:8,color:C.mt,textTransform:"uppercase",marginBottom:3}}>{f.l}</div><input type="number" value={nf[f.k]} onChange={e=>setNf(p=>({...p,[f.k]:e.target.value}))} style={inp}/></div>))}</div><div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:10}}>{["Protein","Carb","Fat","Snack","Meal","Misc"].map(c=>(<button key={c} onClick={()=>setNf(p=>({...p,category:c}))} style={{padding:"3px 8px",borderRadius:12,border:`1px solid ${nf.category===c?C.ac:C.bd}`,background:nf.category===c?C.ac+"12":"transparent",color:nf.category===c?C.ac:C.mt,fontSize:10,cursor:"pointer"}}>{c}</button>))}</div><button onClick={saveNewFood} style={{width:"100%",padding:"10px",background:C.ac,border:"none",borderRadius:8,color:C.bg,fontSize:13,fontWeight:700,cursor:"pointer"}}>Save Food</button></div>)}
    {showS&&(<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:12,marginBottom:12}}><input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search..." style={{...inpL,marginBottom:8}} autoFocus/><div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>{["All","Protein","Carb","Fat","RTD Protein","Snack","Meal","Misc"].map(c=>(<button key={c} onClick={()=>setCat(c)} style={{padding:"3px 8px",borderRadius:12,border:`1px solid ${cat===c?C.ac:C.bd}`,background:cat===c?C.ac+"12":"transparent",color:cat===c?C.ac:C.mt,fontSize:10,cursor:"pointer"}}>{c}</button>))}</div><div style={{maxHeight:220,overflowY:"auto"}}>{flt.length===0?<div style={{padding:16,textAlign:"center",color:C.mt,fontSize:12}}>No foods found</div>:flt.map(f=>(<button key={f.id} onClick={()=>add(f)} style={{width:"100%",padding:"8px 6px",background:"none",border:"none",borderBottom:`1px solid ${C.bd}`,color:C.tx,cursor:"pointer",textAlign:"left",display:"flex",justifyContent:"space-between"}}><div><div style={{fontSize:12}}>{f.name}</div><div style={{fontSize:10,color:C.mt}}>{f.portion_size} {f.portion_unit}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:11,fontFamily:mono,color:C.gn}}>{f.protein_g}p</div><div style={{fontSize:9,color:C.mt}}>{f.calories}cal</div></div></button>))}</div></div>)}
    {log.length>0&&(<div><div style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase",marginBottom:6}}>Today</div>{log.map((m,i)=>(<div key={m.id||i} style={{background:C.sf,borderRadius:8,border:`1px solid ${C.bd}`,padding:"8px 12px",marginBottom:5,display:"flex",alignItems:"center",gap:8}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:500,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{m.food}</div><div style={{fontSize:10,color:C.mt,fontFamily:mono}}>{Math.round(m.protein*m.portions)}p·{Math.round(m.carbs*m.portions)}c·{Math.round(m.fat*m.portions)}f·{Math.round(m.calories*m.portions)}</div></div><div style={{display:"flex",alignItems:"center",gap:3}}><button onClick={()=>up(i,m.portions-0.5)} style={tbtn}>-</button><span style={{fontSize:12,fontFamily:mono,width:24,textAlign:"center"}}>{m.portions}</span><button onClick={()=>up(i,m.portions+0.5)} style={tbtn}>+</button></div><button onClick={()=>rm(i)} style={{background:"none",border:"none",color:C.rd,fontSize:14,cursor:"pointer",padding:"2px"}}>×</button></div>))}</div>)}
    {log.length===0&&!showS&&!showAdd&&<div style={{textAlign:"center",padding:"36px 20px",color:C.mt,fontSize:13}}>No meals logged today</div>}
  </div>);
}

function Body({meas,onAdd,online,onPC}){const[showF,setShowF]=useState(false);const fields=[{k:"measure_date",l:"Date",t:"date"},{k:"bodyweight_lb",l:"Weight (lb)",t:"number"},{k:"chest_in",l:"Chest",t:"number"},{k:"waist_in",l:"Waist *",t:"number"},{k:"hips_in",l:"Hips",t:"number"},{k:"r_arm_in",l:"R Arm",t:"number"},{k:"l_arm_in",l:"L Arm",t:"number"},{k:"r_forearm_in",l:"R Forearm",t:"number"},{k:"l_forearm_in",l:"L Forearm",t:"number"},{k:"shoulder_circ_in",l:"Shoulders",t:"number"},{k:"thigh_in",l:"Thigh",t:"number"},{k:"calf_in",l:"Calf",t:"number"},{k:"neck_in",l:"Neck *",t:"number"}];const init={};fields.forEach(f=>init[f.k]=f.k==="measure_date"?new Date().toISOString().split("T")[0]:"");const[fm,setFm]=useState(init);
  ...
  const lat=meas[meas.length-1];const prev=meas.length>1?meas[meas.length-2]:null;function delta(c,p){if(!c||!p)return null;const d=(c-p).toFixed(1);return parseFloat(d)>0?`+${d}`:d;}
  const latBF=lat?.body_fat_pct||(lat?.waist_in&&lat?.neck_in&&lat?.height_in?navyBF(lat.waist_in,lat.neck_in,lat.height_in):null);
  return(<div style={{padding:"24px 16px"}}><div style={{fontSize:20,fontWeight:700,marginBottom:16}}>Body</div>
    {lat&&(<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:16,marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase"}}>{lat.measure_date}</div>{latBF&&<div style={{background:C.ac+"12",border:`1px solid ${C.ac}22`,borderRadius:6,padding:"4px 10px"}}><span style={{fontSize:10,color:C.mt}}>BF </span><span style={{fontSize:14,fontWeight:700,fontFamily:mono,color:C.ac}}>{latBF}%</span></div>}</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>{[{l:"Weight",v:lat.bodyweight_lb,p:prev?.bodyweight_lb,u:"lb"},{l:"Chest",v:lat.chest_in,p:prev?.chest_in,u:"\""},{l:"Waist",v:lat.waist_in,p:prev?.waist_in,u:"\""},{l:"Hips",v:lat.hips_in,p:prev?.hips_in,u:"\""},{l:"R Arm",v:lat.r_arm_in,p:prev?.r_arm_in,u:"\""},{l:"L Arm",v:lat.l_arm_in,p:prev?.l_arm_in,u:"\""},{l:"R Fore",v:lat.r_forearm_in,p:prev?.r_forearm_in,u:"\""},{l:"L Fore",v:lat.l_forearm_in,p:prev?.l_forearm_in,u:"\""},{l:"Shoulders",v:lat.shoulder_circ_in,p:prev?.shoulder_circ_in,u:"\""},{l:"Thigh",v:lat.thigh_in,p:prev?.thigh_in,u:"\""},{l:"Calf",v:lat.calf_in,p:prev?.calf_in,u:"\""},{l:"Neck",v:lat.neck_in,p:prev?.neck_in,u:"\""}].map(s=>{const d=delta(s.v,s.p);return(<div key={s.l}><div style={{fontSize:8,color:C.mt,textTransform:"uppercase"}}>{s.l}</div><div style={{fontSize:16,fontWeight:700,fontFamily:mono}}>{s.v??"—"}</div>{s.v&&<div style={{fontSize:9,color:C.mt}}>{s.u}{d&&<span style={{color:parseFloat(d)>0?C.gn:C.rd,marginLeft:3}}>{d}</span>}</div>}</div>);})}</div>
    </div>)}
    ...
    {showF&&(<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:14,marginBottom:12}}><div style={{fontSize:10,color:C.mt,marginBottom:8}}>BF% auto-calculates from waist + neck</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{fields.map(f=>(<div key={f.k}><label style={{fontSize:8,color:f.k==="waist_in"||f.k==="neck_in"?C.ac:C.mt,textTransform:"uppercase"}}>{f.l}</label><input type={f.t} inputMode={f.t==="number"?"decimal":undefined} value={fm[f.k]} onChange={e=>setFm(p=>({...p,[f.k]:e.target.value}))} style={{...inp,width:"100%",marginTop:3,fontSize:13}}/></div>))}</div>{fm.waist_in&&fm.neck_in&&<div style={{marginTop:8,padding:"6px 10px",background:C.ac+"08",borderRadius:6,fontSize:12,color:C.ac,textAlign:"center"}}>Est. BF: {navyBF(parseFloat(fm.waist_in),parseFloat(fm.neck_in),meas[meas.length-1]?.height_in||70)||"—"}%</div>}<button onClick={save} style={{width:"100%",padding:"10px",marginTop:10,background:C.ac,border:"none",borderRadius:8,color:C.bg,fontSize:13,fontWeight:700,cursor:"pointer"}}>Save</button></div>)}
...
function Stats({meas,week,online}){const[prs,setPrs]=useState([]);const[vol,setVol]=useState({});const[view,setView]=useState("prs");
  ...
  const wd=meas.filter(m=>m.bodyweight_lb);const allM=[...new Set([...Object.keys(VOL_TARGETS),...Object.keys(vol)])];const volD=allM.map(m=>({muscle:m,actual:vol[m]||0,min:VOL_TARGETS[m]?.min||0,max:VOL_TARGETS[m]?.max||20})).sort((a,b)=>b.actual-a.actual);const maxB=Math.max(...volD.map(v=>Math.max(v.actual,v.max)),1);
  return(<div style={{padding:"24px 16px"}}><div style={{fontSize:20,fontWeight:700,marginBottom:4}}>Stats</div>
    ...
    {view==="vol"&&(<div><div style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase",marginBottom:10}}>Sets vs Target — W{week}</div>{volD.map(v=>{const inR=v.actual>=v.min&&v.actual<=v.max,over=v.actual>v.max,under=v.actual>0&&v.actual<v.min;const bc=v.actual===0?C.mt+"33":inR?C.gn:over?C.am:C.rd;return(<div key={v.muscle} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:11}}>{v.muscle}</span><span style={{fontSize:10,fontFamily:mono,color:bc}}>{v.actual} <span style={{color:C.mt}}>/{v.min}–{v.max}</span></span></div><div style={{position:"relative",width:"100%",height:12,background:C.sf,borderRadius:3,overflow:"hidden"}}><div style={{position:"absolute",left:`${(v.min/maxB)*100}%`,width:`${((v.max-v.min)/maxB)*100}%`,height:"100%",background:C.mt+"15",borderRadius:3}}/><div style={{position:"relative",width:`${(v.actual/maxB)*100}%`,height:"100%",background:bc+"66",borderRadius:3}}/></div></div>);})}<div style={{display:"flex",gap:12,marginTop:12,fontSize:10,color:C.mt}}><span><span style={{color:C.gn}}>●</span> In range</span><span><span style={{color:C.rd}}>●</span> Below</span><span><span style={{color:C.am}}>●</span> Above</span></div></div>)}
    {view==="bw"&&wd.length>1&&(<div><div style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase",marginBottom:10}}>Bodyweight Trend</div><div style={{display:"flex",alignItems:"flex-end",gap:4,height:80}}>{wd.map(m=>{const mn=Math.min(...wd.map(d=>d.bodyweight_lb)),mx=Math.max(...wd.map(d=>d.bodyweight_lb)),rn=mx-mn||1,h=((m.bodyweight_lb-mn)/rn)*60+10;return(<div key={m.id} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}><span style={{fontSize:9,fontFamily:mono,color:C.tx}}>{m.bodyweight_lb}</span><div style={{width:"100%",height:h,background:C.ac+"55",borderRadius:3,maxWidth:32}}/><span style={{fontSize:7,color:C.mt}}>{m.measure_date?.slice(5)}</span></div>);})}</div></div>)}
    {view==="prs"&&(<>{prs.length>0?(<><div style={{display:"grid",gridTemplateColumns:"3fr 1fr 1fr 1fr",marginBottom:6}}>{["Exercise","Best","Reps","E1RM"].map(h=><div key={h} style={{fontSize:9,fontWeight:600,color:C.mt,textTransform:"uppercase",paddingBottom:6,borderBottom:`1px solid ${C.bd}`}}>{h}</div>)}</div>{prs.map(pr=>(<div key={pr.exercise} style={{display:"grid",gridTemplateColumns:"3fr 1fr 1fr 1fr",padding:"7px 0",borderBottom:`1px solid ${C.bd}`}}><div style={{fontSize:11,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",paddingRight:6}}>{pr.exercise}</div><div style={{fontSize:11,fontFamily:mono,color:C.ac}}>{pr.weight}</div><div style={{fontSize:11,fontFamily:mono,color:C.mt}}>{pr.reps}</div><div style={{fontSize:11,fontFamily:mono,color:C.gn}}>{Math.round(pr.est1rm)}</div></div>))}</>):(<div style={{textAlign:"center",padding:"36px 20px",color:C.mt,fontSize:13}}>No PRs yet</div>)}</>)}
  </div>);
}
