...
function Timer({duration,onDismiss}){const[rem,setRem]=useState(duration);const[on,setOn]=useState(true);const ref=useRef(null);
  ...
  return(<div style={{background:done?C.gn+"12":C.sf2,border:`1px solid ${done?C.gn+"33":C.bd}`,borderRadius:10,padding:"10px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:12}}>
    ...
    <button onClick={onDismiss} style={{padding:"5px 10px",background:done?C.gn+"15":C.sf,border:`1px solid ${done?C.gn+"33":C.bd}`,borderRadius:6,color:done?C.gn:C.mt,fontSize:11,fontWeight:600,cursor:"pointer"}}>{done?"Continue":"Skip"}</button></div></div>);
}
...
export default function App(){
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",color:C.tx,fontFamily:sans,maxWidth:480,margin:"0 auto",paddingBottom:80}}>
      ...
      {/* NAV BAR - bigger targets, line icons */}
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:C.sf,borderTop:`1px solid ${C.bd}`,display:"flex",zIndex:100,padding:"6px 0 env(safe-area-inset-bottom, 4px)"}}>
        {tabs.map(t=>{const active=tab===t.id;const color=active?C.ac:C.mt;return(
          <button key={t.id} onClick={()=>{setTab(t.id);if(t.id!=="train")setSelDay(null);}}
            ...
            <t.Icon c={color}/>
            <span style={{fontSize:10,fontWeight:active?700:500,color,letterSpacing:"0.04em",fontFamily:sans}}>{t.label}</span>
          </button>
        );})}
      </div>
    </div>);
...
function DaySelect({days,onSelect,week,setWeek,restDur,setRestDur,weekType,setWeekType,online}){
  const[showCfg,setShowCfg]=useState(false);const[wc,setWc]=useState(null);const[summary,setSummary]=useState(null);
  ...
  async function loadSummary(){if(!online)return;try{
    const{data}=await supabase.from("workout_sessions").select("id,training_day_id,workout_sets(exercise_id,weight_lb,reps,exercises(name,primary_muscle))").eq("week_number",week);
    if(!data?.length){setSummary(null);return;}
    let totalSets=0,completedSets=0,prCount=0;const muscles={};
    // Get previous week for PR comparison
    const{data:prevData}=await supabase.from("workout_sessions").select("id,workout_sets(exercise_id,weight_lb,reps,exercises(name))").eq("week_number",week-1);
    ...
    const sessionsLogged=data.length;
    setSummary({totalSets,completedSets,prCount,sessionsLogged,muscles});
  }catch{}}

  const isDL=weekType==="Deload";const[showSum,setShowSum]=useState(false);
  return(<div style={{padding:"24px 16px"}}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
      <span style={{fontSize:12,color:isDL?C.am:C.mt}}>{weekType}</span>
      {wc!==null&&<span style={{fontSize:11,fontFamily:mono,color:wc===100?C.gn:wc>0?C.am:C.mt}}>{wc}%</span>}
      {summary&&<button onClick={()=>setShowSum(!showSum)} style={{marginLeft:"auto",padding:"4px 10px",background:C.sf,border:`1px solid ${C.bd}`,borderRadius:6,color:showSum?C.ac:C.mt,fontSize:10,cursor:"pointer"}}>Summary</button>}
    </div>
    ...
    {showSum&&summary&&(
      <div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:14,marginBottom:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:12}}>
          {[{l:"Sessions",v:summary.sessionsLogged,c:C.ac},{l:"Sets Done",v:summary.completedSets,c:C.bl},{l:"Total Sets",v:summary.totalSets,c:C.mt},{l:"New PRs",v:summary.prCount,c:summary.prCount>0?C.gn:C.mt}].map(s=>
            <div key={s.l} style={{textAlign:"center"}}><div style={{fontSize:18,fontWeight:700,fontFamily:mono,color:s.c}}>{s.v}</div><div style={{fontSize:8,color:C.mt,textTransform:"uppercase",marginTop:2}}>{s.l}</div></div>
          )}
        </div>
        {Object.keys(summary.muscles).length>0&&<div>
          <div style={{fontSize:9,color:C.mt,textTransform:"uppercase",marginBottom:6}}>Volume by Muscle</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
            {Object.entries(summary.muscles).sort((a,b)=>b[1]-a[1]).map(([m,sets])=>{
              const tgt=VOL_TARGETS[m];const inR=tgt&&sets>=tgt.min&&sets<=tgt.max;
              return <span key={m} style={{padding:"3px 8px",borderRadius:4,background:C.sf2,border:`1px solid ${inR?C.gn+"33":C.bd}`,fontSize:10,fontFamily:mono}}><span style={{color:C.tx}}>{m}</span> <span style={{color:inR?C.gn:C.mt}}>{sets}</span></span>;
            })}
          </div>
        </div>}
      </div>
...
function Session({day,onBack,week,restDur,weekType,isDeload,online,onPC}){
  const[expEx,setExpEx]=useState(0);const[showCues,setShowCues]=useState(null);const[sd,setSd]=useState({});const[sid,setSid]=useState(null);const[saved,setSaved]=useState(null);const[timer,setTimer]=useState(false);const[tk,setTk]=useState(0);const[lw,setLw]=useState({});const[history,setHistory]=useState(null);
  ...
  async function loadHistory(exerciseId){
    try{const{data}=await supabase.from("workout_sets").select("weight_lb,reps,workout_sessions(week_number)").eq("exercise_id",exerciseId).order("created_at",{ascending:true});
    if(data){const byWeek={};data.forEach(s=>{const wk=s.workout_sessions?.week_number;if(!wk)return;if(!byWeek[wk])byWeek[wk]={maxW:0,totalReps:0,sets:0};byWeek[wk].maxW=Math.max(byWeek[wk].maxW,s.weight_lb||0);byWeek[wk].totalReps+=s.reps||0;byWeek[wk].sets++;});
    const weeks=Object.entries(byWeek).map(([wk,d])=>({week:parseInt(wk),weight:d.maxW,avgReps:d.sets>0?(d.totalReps/d.sets).toFixed(1):0})).sort((a,b)=>a.week-b.week);
    setHistory({exerciseId,weeks});}}catch{}}
    ...
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      {day.exercises.map((ex,xi)=>{const es=eff(ex),isE=expEx===xi,dn=done(ex.id,es),all=dn===es,pg=lw[ex.id];
        return(<div key={ex.id} style={{background:C.sf,borderRadius:10,border:`1px solid ${all?C.gn+"28":C.bd}`,overflow:"hidden"}}>
          <button onClick={()=>{setExpEx(isE?-1:xi);setHistory(null);}} style={{width:"100%",padding:"12px 14px",background:"none",border:"none",color:C.tx,cursor:"pointer",display:"flex",alignItems:"center",gap:10,textAlign:"left"}}>
          ...
          {isE&&(<div style={{padding:"0 14px 14px"}}>
              ...
              <div style={{background:C.sf2,borderRadius:8,padding:10,marginBottom:8}}>
                ...
                <div style={{display:"flex",alignItems:"flex-end",gap:3,height:50}}>
                  {history.weeks.slice(-12).map(w=>{const mn=Math.min(...history.weeks.map(x=>x.weight)),mx=Math.max(...history.weeks.map(x=>x.weight)),rn=mx-mn||1,h=((w.weight-mn)/rn)*40+8;
                    return(<div key={w.week} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                      <span style={{fontSize:7,fontFamily:mono,color:C.tx}}>{w.weight}</span>
                      <div style={{width:"100%",height:h,background:w.week===week?C.ac:C.ac+"44",borderRadius:2,maxWidth:24}}/>
                      <span style={{fontSize:6,color:C.mt}}>W{w.week}</span>
                    </div>);})}
                </div>
                <div style={{display:"flex",justifyContent:"space-between",marginTop:6,fontSize:9,color:C.mt}}>
                  <span>Avg reps: {history.weeks[history.weeks.length-1]?.avgReps}</span>
                  <span>{history.weeks.length} weeks tracked</span>
                </div>
              </div>
            )}
            ...
            {xi<day.exercises.length-1&&<button onClick={()=>{setExpEx(xi+1);setTimer(false);setHistory(null);}} style={{width:"100%",padding:"9px",marginTop:4,background:C.ac+"10",border:`1px solid ${C.ac}25`,borderRadius:8,color:C.ac,fontSize:12,fontWeight:600,cursor:"pointer"}}>Next exercise</button>}
          </div>)}
        </div>);
      })}
    </div>
...
function Fuel({foods,setFoods,mt,setMt,online,onPC}){
  ...
  const[showCalc,setShowCalc]=useState(false);const[showAdd,setShowAdd]=useState(false);const[showRecent,setShowRecent]=useState(false);
  ...
  async function loadRecent(){try{
    // Get yesterday's and day-before meals for quick-add
    const yesterday=new Date(Date.now()-86400000).toISOString().split("T")[0];
    const{data}=await supabase.from("meal_log").select("food_id,portions,foods(*)").gte("log_date",yesterday).order("created_at",{ascending:false}).limit(20);
    if(data){const seen=new Set();const unique=[];data.forEach(m=>{if(m.foods&&!seen.has(m.food_id)){seen.add(m.food_id);unique.push({...m.foods,lastPortions:parseFloat(m.portions)});}});setRecentFoods(unique);cache.set("recent_foods",unique);}
  }catch{const c=cache.get("recent_foods");if(c)setRecentFoods(c);}}
  ...
  return(<div style={{padding:"24px 16px"}}>
    ...
    {recentFoods.length>0&&!showS&&!showAdd&&(
      <div style={{marginBottom:12}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
          <span style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase"}}>Recent</span>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {(showRecent?recentFoods:recentFoods.slice(0,6)).map(f=>(
            <button key={f.id} onClick={()=>add(f,f.lastPortions||1)} style={{padding:"6px 10px",background:C.sf,border:`1px solid ${C.bd}`,borderRadius:8,color:C.tx,fontSize:11,cursor:"pointer",textAlign:"left",display:"flex",gap:6,alignItems:"center"}}>
              <span>{f.name.length>18?f.name.slice(0,18)+"…":f.name}</span>
              <span style={{fontFamily:mono,fontSize:10,color:C.gn}}>{f.protein_g}p</span>
            </button>
          ))}
        </div>
      </div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:12}}>
      <button onClick={()=>{setShowS(!showS);setShowAdd(false);}} style={{flex:1,padding:"10px",background:showS?C.sf2:C.ac+"10",border:`1px solid ${showS?C.bd:C.ac}25`,borderRadius:8,color:showS?C.mt:C.ac,fontSize:13,fontWeight:600,cursor:"pointer"}}>{showS?"Close":"Search foods"}</button>
      <button onClick={()=>{setShowAdd(!showAdd);setShowS(false);}} style={{padding:"10px 14px",background:showAdd?C.sf2:C.sf,border:`1px solid ${showAdd?C.ac:C.bd}`,borderRadius:8,color:showAdd?C.ac:C.mt,fontSize:12,cursor:"pointer"}}>+ New</button>
    </div>
    {showAdd&&(<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:14,marginBottom:12}}>
      <div style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase",marginBottom:10}}>Custom Food</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,marginBottom:8}}>
        {[{k:"protein_g",l:"Pro"},{k:"carbs_g",l:"Carb"},{k:"fat_g",l:"Fat"},{k:"calories",l:"Cal"}].map(f=>(<div key={f.k}><div style={{fontSize:8,color:C.mt,textTransform:"uppercase",marginBottom:3}}>{f.l}</div><input type="number" value={nf[f.k]} onChange={e=>setNf(p=>({...p,[f.k]:e.target.value}))} style={inp}/></div>))}
      </div>
...
function Body({meas,onAdd,online,onPC}){
  ...
  return(<div style={{padding:"24px 16px"}}>
    ...
    {showF&&(<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:14,marginBottom:12}}>
      <div style={{fontSize:10,color:C.mt,marginBottom:8}}>BF% auto-calculates from waist + neck</div>
...
function Stats({meas,week,online}){
  ...
  const wd=meas.filter(m=>m.bodyweight_lb);const allM=[...new Set([...Object.keys(VOL_TARGETS),...Object.keys(vol)])];
