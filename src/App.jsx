...
// Pending write queue for offline sync
...
function clearPending() { cache.set("pending", []); }
  ...
  for (const op of q) {
    try {
      if (op.type === "upsert_set") {
        if (op.dbId) {
          await supabase.from("workout_sets").update({ weight_lb: op.weight, reps: op.reps }).eq("id", op.dbId);
        } else {
          await supabase.from("workout_sets").insert({ session_id: op.sessionId, exercise_id: op.exerciseId, set_number: op.setNumber, weight_lb: op.weight, reps: op.reps });
        }
        synced++;
      } else if (op.type === "insert_meal") {
        await supabase.from("meal_log").insert({ log_date: op.date, food_id: op.foodId, portions: op.portions });
        synced++;
      } else if (op.type === "delete_meal") {
        await supabase.from("meal_log").delete().eq("id", op.id);
        synced++;
      } else if (op.type === "update_portions") {
        await supabase.from("meal_log").update({ portions: op.portions }).eq("id", op.id);
        synced++;
      } else if (op.type === "insert_measurement") {
        await supabase.from("measurements").insert(op.data);
        synced++;
      } else if (op.type === "create_session") {
        await supabase.from("workout_sessions").insert(op.data);
        synced++;
      }
    } catch { failed.push(op); }
  }
  cache.set("pending", failed);
  return synced;
}
...
export default function App() {
  ...
  // When coming back online, flush pending writes
  useEffect(() => {
    if (online) {
      flushPending().then(n => {
        if (n > 0) { setPendingCount(getPending().length); load(); }
      });
    }
  }, [online]);
  ...
  async function load() {
    ...
    try {
      const { data: d, error: dErr } = await supabase.from("training_days").select("*, training_day_exercises(*, exercises(*))").order("day_order");
      if (dErr) throw dErr;
      if (d) {
        const formatted = d.map(x => ({ id:x.id, name:x.name, focus:x.focus, exercises:(x.training_day_exercises||[]).sort((a,b)=>a.exercise_order-b.exercise_order).map(t=>({ id:t.exercises.id, name:t.exercises.name, sets:t.default_sets, repMin:t.exercises.rep_min, repMax:t.exercises.rep_max, increment:parseFloat(t.exercises.increment_lb)||2.5, category:t.exercises.category, cues:t.exercises.cues, muscle:t.exercises.primary_muscle, video:t.exercises.video_url })) }));
        setDays(formatted);
        cache.set("days", formatted);
      }
      const { data: fd } = await supabase.from("foods").select("*").order("name");
      if (fd) { setFoods(fd); cache.set("foods", fd); }
      const { data: tg } = await supabase.from("macro_targets").select("*").eq("is_active",true).limit(1);
      if (tg?.[0]) { const t = { protein:tg[0].protein_g_target, carbs:tg[0].carbs_g_target, fat:tg[0].fat_g_target, calories:tg[0].calories_target }; setMt(t); cache.set("mt", t); }
      const { data: ms } = await supabase.from("measurements").select("*").order("measure_date");
      if (ms) { setMeas(ms); cache.set("meas", ms); }
    } catch {
      // Offline — load from cache
      usedCache = true;
      const cd = cache.get("days"); if (cd) setDays(cd);
      const cf = cache.get("foods"); if (cf) setFoods(cf);
      const cm = cache.get("mt"); if (cm) setMt(cm);
      const cmeas = cache.get("meas"); if (cmeas) setMeas(cmeas);
    }
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", color:C.tx, fontFamily:sans, maxWidth:480, margin:"0 auto", paddingBottom:72 }}>
      ...
      {(!online || pendingCount > 0) && (
        <div style={{ background:!online?C.am+"15":C.gn+"12", borderBottom:`1px solid ${!online?C.am+"33":C.gn+"33"}`, padding:"6px 16px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <span style={{ fontSize:11, color:!online?C.am:C.gn }}>{!online ? "Offline — changes saved locally" : `Syncing ${pendingCount} pending...`}</span>
          {!online && <span style={{ fontSize:9, fontFamily:mono, color:C.am }}>{pendingCount > 0 ? `${pendingCount} queued` : ""}</span>}
        </div>
      )}
      ...
      {tab==="train" && selDay && <Session day={selDay} onBack={()=>setSelDay(null)} week={week} restDur={restDur} weekType={weekType} isDeload={weekType==="Deload"} online={online} onPendingChange={()=>setPendingCount(getPending().length)} />}
      {tab==="fuel" && <Fuel foods={foods} mt={mt} setMt={setMt} online={online} onPendingChange={()=>setPendingCount(getPending().length)} />}
      {tab==="body" && <Body meas={meas} onAdd={m=>setMeas(p=>[...p,m].sort((a,b)=>a.measure_date.localeCompare(b.measure_date)))} online={online} onPendingChange={()=>setPendingCount(getPending().length)} />}
      {tab==="stats" && <Stats meas={meas} days={days} week={week} online={online} />}
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, background:C.sf, borderTop:`1px solid ${C.bd}`, display:"flex", zIndex:100 }}>
        {["train","fuel","body","stats"].map(t=>(
          <button key={t} onClick={()=>{setTab(t);if(t!=="train")setSelDay(null);}} style={{ flex:1, padding:"12px 0 10px", background:"none", border:"none", color:tab===t?C.ac:C.mt, fontSize:11, fontWeight:600, letterSpacing:"0.06em", textTransform:"uppercase", cursor:"pointer", fontFamily:sans }}>{t}</button>
        ))}
      </div>
    </div>
  );
...
function DaySelect({ days, onSelect, week, setWeek, restDur, setRestDur, weekType, setWeekType, online }) {
  ...
  useEffect(() => { loadComp(); }, [week]);
  async function loadComp() {
    if (!online) { setWeekComp(null); return; }
    const { data } = await supabase.from("workout_sessions").select("id, training_day_id, workout_sets(reps)").eq("week_number",week);
    if (!data?.length) { setWeekComp(null); return; }
    const totalP = days.reduce((s,d)=>s+d.exercises.reduce((s2,e)=>s2+e.sets,0),0);
    let done = 0; data.forEach(s=>s.workout_sets.forEach(ws=>{if(ws.reps>0)done++;}));
    setWeekComp(totalP>0?Math.round((done/totalP)*100):0);
  }
  const isDL = weekType==="Deload";
  return (
    <div style={{ padding:"24px 16px" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:2 }}>
        <div style={{ fontSize:20, fontWeight:700 }}>IRON<span style={{ color:C.ac }}>LOG</span></div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
        <span style={{ fontSize:12, color:isDL?C.am:C.mt }}>{weekType}{isDL?" — reduced volume":""}</span>
        {weekComp!==null && <span style={{ fontSize:11, fontFamily:mono, color:weekComp===100?C.gn:weekComp>0?C.am:C.mt }}>{weekComp}%</span>}
      </div>
      {showCfg && (
        <div style={{ background:C.sf, borderRadius:10, border:`1px solid ${C.bd}`, padding:14, marginBottom:14 }}>
          <div style={{ marginBottom:12 }}>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {WEEK_TYPES.map(t=>(<button key={t} onClick={()=>setWeekType(t)} style={{ padding:"6px 12px", borderRadius:6, border:`1px solid ${weekType===t?t==="Deload"?C.am:C.ac:C.bd}`, background:weekType===t?(t==="Deload"?C.am:C.ac)+"15":"transparent", color:weekType===t?(t==="Deload"?C.am:C.ac):C.mt, fontSize:12, cursor:"pointer" }}>{t}</button>))}
            </div>
            {isDL && <div style={{ fontSize:11, color:C.am, marginTop:8 }}>Deload: 2 sets per exercise at ~60% weight</div>}
          </div>
          <div>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {[60,90,120,150,180,240].map(t=>(<button key={t} onClick={()=>setRestDur(t)} style={{ padding:"6px 12px", borderRadius:6, border:`1px solid ${restDur===t?C.ac:C.bd}`, background:restDur===t?C.ac+"15":"transparent", color:restDur===t?C.ac:C.mt, fontSize:12, fontFamily:mono, cursor:"pointer" }}>{Math.floor(t/60)}:{String(t%60).padStart(2,"0")}</button>))}
            </div>
          </div>
        </div>
      )}
...
function Session({ day, onBack, week, restDur, weekType, isDeload, online, onPendingChange }) {
    ...
    try {
      ...
      if (data?.[0]) {
        ...
        Object.entries(byE).forEach(([eid, sets]) => {
          const v = sets.filter(s=>s.reps>0&&s.weight_lb>0); if (!v.length) return;
          ...
          if (isDeload) { prog[eid] = { w:mw, r:avg, up:false, sw:Math.round(mw*0.6/2.5)*2.5, deload:true }; }
          else { const hit = ex && avg >= ex.repMax; prog[eid] = { w:mw, r:avg, up:hit, sw:hit&&ex?mw+ex.increment:mw }; }
        });
  ...
  async function init() {
    const cacheKey = `session_${day.id}_${week}`;
    try {
      ...
      if (data?.[0]) {
        setSid(data[0].id);
        const l = {}; data[0].workout_sets.forEach(w => { l[`${w.exercise_id}-${w.set_number}`] = { weight:w.weight_lb||0, reps:w.reps||0, dbId:w.id }; });
        setSd(l); cache.set(cacheKey, { sid: data[0].id, sets: l });
      } else {
        ...
        if (n) { setSid(n.id); cache.set(cacheKey, { sid: n.id, sets: {} }); }
      }
    } catch {
      const c = cache.get(cacheKey);
      if (c) { setSid(c.sid); setSd(c.sets); }
      else {
        // Fully offline, create a temp session ID
        const tempId = `temp_${Date.now()}`;
        setSid(tempId);
        addPending({ type:"create_session", data:{ week_number:week, training_day_id:day.id, session_date:new Date().toISOString().split("T")[0], week_type:weekType } });
        onPendingChange();
      }
    }
  }
  ...
  async function sv(eid,sn) {
    if (!sid) return;
    const k=`${eid}-${sn}`, d=sd[k]; if(!d||(!d.weight&&!d.reps)) return;

    // Always save locally first
    const cacheKey = `session_${day.id}_${week}`;
    const cached = cache.get(cacheKey) || { sid, sets: {} };
    cached.sets[k] = { weight:d.weight, reps:d.reps, dbId:d.dbId };
    cache.set(cacheKey, cached);

    // Try remote save
    try {
      if (d.dbId) {
        await supabase.from("workout_sets").update({weight_lb:d.weight,reps:d.reps}).eq("id",d.dbId);
      } else {
        const{data:ins}=await supabase.from("workout_sets").insert({session_id:sid,exercise_id:eid,set_number:sn,weight_lb:d.weight,reps:d.reps}).select().single();
        if(ins) { setSd(p=>({...p,[k]:{...p[k],dbId:ins.id}})); cached.sets[k].dbId=ins.id; cache.set(cacheKey,cached); }
      }
    } catch {
      // Queue for later sync
      addPending({ type:"upsert_set", dbId:d.dbId, sessionId:sid, exerciseId:eid, setNumber:sn, weight:d.weight, reps:d.reps });
      onPendingChange();
    }
  ...
  const totalSets = day.exercises.reduce((s,e)=>s+eff(e),0);
  const doneSets = day.exercises.reduce((s,e)=>s+done(e.id,eff(e)),0);
  ...
  return (
    <div style={{ padding:"20px 16px" }}>
      ...
      {isDeload && <div style={{ padding:"8px 12px", marginBottom:8, marginTop:8, background:C.am+"10", border:`1px solid ${C.am}22`, borderRadius:8, fontSize:11, color:C.am }}>Deload: 2 sets at ~60% intensity. Focus on form.</div>}
            ...
            <div key={ex.id} style={{ background:C.sf, borderRadius:10, border:`1px solid ${all?C.gn+"28":C.bd}`, overflow:"hidden" }}>
              <button onClick={()=>setExpEx(isE?-1:xi)} style={{ width:"100%", padding:"12px 14px", background:"none", border:"none", color:C.tx, cursor:"pointer", display:"flex", alignItems:"center", gap:10, textAlign:"left" }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:11, color:C.mt, marginTop:1 }}>{es} × {ex.repMin}–{ex.repMax}{isDeload&&ex.sets>es&&<span style={{color:C.am}}> (from {ex.sets})</span>}{dn>0&&<span style={{color:all?C.gn:C.am}}> · {dn}/{es}</span>}</div>
                </div>
              ...
              {isE && (
                <div style={{ padding:"0 14px 14px" }}>
                  <div style={{ display:"flex", gap:6, marginBottom:8 }}>
                    <button onClick={()=>setShowCues(showCues===xi?null:xi)} style={{ flex:1, padding:"7px 10px", background:C.sf2, border:`1px solid ${C.bd}`, borderRadius:6, color:C.mt, fontSize:11, cursor:"pointer", textAlign:"left" }}>{showCues===xi?ex.cues:"View setup cues"}</button>
                  ...
                  {pg?.deload && <div style={{ padding:"7px 10px", marginBottom:8, background:C.am+"08", border:`1px solid ${C.am}22`, borderRadius:6, fontSize:11 }}><span style={{color:C.am}}>Deload: </span><span style={{color:C.tx}}>Last {pg.w}lb. </span><span style={{color:C.am,fontWeight:600}}>Use {pg.sw}lb (~60%)</span></div>}
                  {pg?.up&&!isDeload && <div style={{ padding:"7px 10px", marginBottom:8, background:C.gn+"08", border:`1px solid ${C.gn}22`, borderRadius:6, fontSize:11 }}><span style={{color:C.gn}}>Progression: </span><span style={{color:C.tx}}>Last week {pg.r.toFixed(1)} avg at {pg.w}lb. </span><span style={{color:C.gn,fontWeight:600}}>Try {pg.sw}lb</span></div>}
                  ...
                  {Array.from({length:es},(_,i)=>{
                    const sn=i+1, s=gs(ex.id,sn), ok=s.reps>0, hi=s.reps>ex.repMax, lo=s.reps>0&&s.reps<ex.repMin;
                    return (<div key={i} style={{ display:"grid", gridTemplateColumns:"32px 1fr 1fr 44px", gap:5, marginBottom:5, alignItems:"center" }}>
...
function Fuel({ foods, mt, setMt, online, onPendingChange }) {
  ...
  async function loadLog() {
    try {
      ...
      if (data) { const l = data.map(m=>({ id:m.id, food:m.foods?.name||"?", portions:parseFloat(m.portions), protein:m.foods?.protein_g||0, carbs:m.foods?.carbs_g||0, fat:m.foods?.fat_g||0, calories:m.foods?.calories||0 })); setLog(l); cache.set(`meals_${td}`, l); }
    } catch { const c = cache.get(`meals_${td}`); if (c) setLog(c); }
  }
  async function add(f) {
    const entry = { id:`temp_${Date.now()}`, food:f.name, portions:1, protein:f.protein_g, carbs:f.carbs_g, fat:f.fat_g, calories:f.calories };
    setLog(p=>{const n=[...p,entry]; cache.set(`meals_${td}`,n); return n;});
    try { const { data:ins } = await supabase.from("meal_log").insert({ log_date:td, food_id:f.id, portions:1 }).select().single(); if(ins) setLog(p=>p.map(m=>m.id===entry.id?{...m,id:ins.id}:m)); }
    catch { addPending({ type:"insert_meal", date:td, foodId:f.id, portions:1 }); onPendingChange(); }
    setShowS(false); setSearch("");
  }
  async function rm(i) { const e=log[i]; setLog(p=>{const n=p.filter((_,x)=>x!==i); cache.set(`meals_${td}`,n); return n;});
    if(e?.id && !String(e.id).startsWith("temp")) { try { await supabase.from("meal_log").delete().eq("id",e.id); } catch { addPending({ type:"delete_meal", id:e.id }); onPendingChange(); } }
  }
  async function up(i,pt) { const np=Math.max(0.25,pt); setLog(p=>{const n=p.map((m,x)=>x===i?{...m,portions:np}:m); cache.set(`meals_${td}`,n); return n;});
    const e=log[i]; if(e?.id && !String(e.id).startsWith("temp")) { try { await supabase.from("meal_log").update({portions:np}).eq("id",e.id); } catch { addPending({ type:"update_portions", id:e.id, portions:np }); onPendingChange(); } }
  }
  ...
  function recalc() {
    ...
    setMt({protein,carbs,fat,calories:cal}); cache.set("mt",{protein,carbs,fat,calories:cal});
  ...
  return (
    <div style={{ padding:"24px 16px" }}>
      ...
      {showCalc && (
        <div style={{ background:C.sf, borderRadius:10, border:`1px solid ${C.bd}`, padding:14, marginBottom:14 }}>
          <div style={{ fontSize:10, fontWeight:600, color:C.mt, textTransform:"uppercase", marginBottom:10 }}>Macro Calculator</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:10 }}>
            <div><div style={{ fontSize:9, color:C.mt, textTransform:"uppercase", marginBottom:4 }}>Weight</div><input type="number" value={calcW} onChange={e=>setCalcW(e.target.value)} style={inp} /></div>
            <div><div style={{ fontSize:9, color:C.mt, textTransform:"uppercase", marginBottom:4 }}>Pro/lb</div><input type="number" value={calcP} onChange={e=>setCalcP(e.target.value)} style={inp} /></div>
...
function Body({ meas, onAdd, online, onPendingChange }) {
  ...
  async function save() {
    ...
    const lat = meas[meas.length-1]; const height = lat?.height_in || 70;
    if (e.waist_in && e.neck_in) e.body_fat_pct = parseFloat(navyBF(e.waist_in, e.neck_in, height));
    e.height_in = height;
    try { const { data } = await supabase.from("measurements").insert(e).select().single(); if (data) { onAdd(data); setShowF(false); } }
    catch { onAdd({ ...e, id:`temp_${Date.now()}` }); addPending({ type:"insert_measurement", data:e }); onPendingChange(); setShowF(false); }
  }
    ...
    <div style={{ padding:"24px 16px" }}>
      ...
      {lat && (<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:16,marginBottom:12}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
          {[{l:"Weight",v:lat.bodyweight_lb,p:prev?.bodyweight_lb,u:"lb"},{l:"Chest",v:lat.chest_in,p:prev?.chest_in,u:"in"},{l:"Waist",v:lat.waist_in,p:prev?.waist_in,u:"in"},{l:"Hips",v:lat.hips_in,p:prev?.hips_in,u:"in"},{l:"R Arm",v:lat.r_arm_in,p:prev?.r_arm_in,u:"in"},{l:"L Arm",v:lat.l_arm_in,p:prev?.l_arm_in,u:"in"}].map(s=>{const d=delta(s.v,s.p); return(<div key={s.l}><div style={{fontSize:9,color:C.mt,textTransform:"uppercase"}}>{s.l}</div><div style={{fontSize:20,fontWeight:700,fontFamily:mono}}>{s.v??"—"}</div><div style={{fontSize:10,color:C.mt}}>{s.u} {d&&<span style={{color:parseFloat(d)>0?C.gn:C.rd}}>{d}</span>}</div></div>);})}</div>
      </div>)}
      ...
      {showF && (<div style={{background:C.sf,borderRadius:10,border:`1px solid ${C.bd}`,padding:14,marginBottom:12}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {[{k:"measure_date",l:"Date",t:"date"},{k:"bodyweight_lb",l:"Weight (lb)",t:"number"},{k:"chest_in",l:"Chest",t:"number"},{k:"waist_in",l:"Waist *",t:"number"},{k:"hips_in",l:"Hips",t:"number"},{k:"r_arm_in",l:"R Arm",t:"number"},{k:"l_arm_in",l:"L Arm",t:"number"},{k:"neck_in",l:"Neck *",t:"number"}].map(f=>(<div key={f.k}><label style={{fontSize:9,color:f.k==="waist_in"||f.k==="neck_in"?C.ac:C.mt,textTransform:"uppercase"}}>{f.l}</label><input type={f.t} inputMode={f.t==="number"?"decimal":undefined} value={fm[f.k]} onChange={e=>setFm(p=>({...p,[f.k]:e.target.value}))} style={{...inp,width:"100%",marginTop:3}} /></div>))}
        </div>
        {fm.waist_in&&fm.neck_in&&<div style={{marginTop:8,padding:"6px 10px",background:C.ac+"08",borderRadius:6,fontSize:12,color:C.ac,textAlign:"center"}}>Estimated BF: {navyBF(parseFloat(fm.waist_in),parseFloat(fm.neck_in),meas[meas.length-1]?.height_in||70)||"—"}%</div>}
...
function Stats({ meas, days, week, online }) {
  ...
  async function loadPRs() {
    try {
      ...
      if (data) { const best={}; data.forEach(s=>{const n=s.exercises?.name;if(!n||!s.weight_lb||!s.reps)return;const e1=s.weight_lb*(1+s.reps/30);if(!best[n]||e1>best[n].est1rm)best[n]={exercise:n,weight:s.weight_lb,reps:s.reps,est1rm:e1};}); const p=Object.values(best).sort((a,b)=>b.est1rm-a.est1rm); setPrs(p); cache.set("prs",p); }
    } catch { const c=cache.get("prs"); if(c) setPrs(c); }
  }
  async function loadVol() {
    try {
      ...
      if (data) { const m={}; data.forEach(s=>s.workout_sets.forEach(ws=>{if(ws.reps>0&&ws.exercises?.primary_muscle){const mu=ws.exercises.primary_muscle;m[mu]=(m[mu]||0)+1;}})); setVol(m); }
    } catch {}
  }
  ...
  return (
    <div style={{ padding:"24px 16px" }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16 }}>
        {[{id:"prs",l:"PRs"},{id:"vol",l:`Vol W${week}`},{id:"bw",l:"Weight"}].map(v=>(<button key={v.id} onClick={()=>setView(v.id)} style={{ padding:"6px 14px", borderRadius:6, border:`1px solid ${view===v.id?C.ac:C.bd}`, background:view===v.id?C.ac+"15":"transparent", color:view===v.id?C.ac:C.mt, fontSize:12, cursor:"pointer" }}>{v.l}</button>))}
      </div>
      {view==="vol" && (<div>
        <div style={{fontSize:10,fontWeight:600,color:C.mt,textTransform:"uppercase",marginBottom:10}}>Sets vs Target — W{week}</div>
        {volD.length===0?<div style={{padding:24,textAlign:"center",color:C.mt,fontSize:12}}>No data</div>:
        volD.map(v=>{const inR=v.actual>=v.min&&v.actual<=v.max, over=v.actual>v.max, under=v.actual>0&&v.actual<v.min; const bc=v.actual===0?C.mt+"33":inR?C.gn:over?C.am:C.rd; const st=v.actual===0?"":inR?"ok":over?"high":under?"low":"";
          return(<div key={v.muscle} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:11,color:C.tx}}>{v.muscle}</span><span style={{fontSize:10,fontFamily:mono,color:bc}}>{v.actual} <span style={{color:C.mt}}>/ {v.min}–{v.max}</span>{st&&<span style={{marginLeft:4,fontSize:9}}>({st})</span>}</span></div>
      ...
      {view==="prs"&&(<>{prs.length>0?(<><div style={{display:"grid",gridTemplateColumns:"3fr 1fr 1fr 1fr",gap:0,marginBottom:6}}>{["Exercise","Best","Reps","E1RM"].map(h=><div key={h} style={{fontSize:9,fontWeight:600,color:C.mt,textTransform:"uppercase",paddingBottom:6,borderBottom:`1px solid ${C.bd}`}}>{h}</div>)}</div>
